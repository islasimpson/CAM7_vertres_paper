...
for ip in pre:
    for ilatbnd in latbnd:
        w = xr.open_mfdataset("/project/cas/islas/python_savs/CAM7_vertres_paper/RAW_DATA/MERRA2/dayfrom6h/"+
           "uvwt_*.nc", preprocess = partial(preprocessor)).sel(plev=ip, lat=slice(-1.*ilatbnd-2, ilatbnd+2)).load() 

        wpower = cospec.cospeccalc(w.w,w.w, lat_bnds=(-1.*ilatbnd, ilatbnd),
        ...
        dout.to_netcdf(pathout+'MERRA2_'+str(ip)+'hpa_'+str(ilatbnd)+'Sto'+str(ilatbnd)+'N.nc') 
