...
for imem in mems:
    print(imem)
    filedir=basepath+'Uzm/'
    uzm_xr = xr.open_mfdataset(filedir+"/*-"+imem+"*.Uzm.*.nc").sel(time=slice("1978-01-01","2024-12-31"))
    uzm_xr = read.fixcesmtime_daily(uzm_xr)
    uzm_xr = uzm_xr.where( uzm_xr.time.dt.hour == 12, drop=True)
    uzm_xr = uzm_xr.sel(time=slice("1979-01-01","2023-12-31"))
    uzm_xr = uzm_xr.chunk({'time':uzm_xr.time.size, 'ilev':uzm_xr.ilev.size, 'lat':uzm_xr.lat.size})
    print('after Uzm')

    filedir=basepath+'THzm/'
    thzm_xr = xr.open_mfdataset(filedir+"/*-"+imem+"*.THzm.*.nc").sel(time=slice("1978-01-01","2024-12-31"))
    thzm_xr = read.fixcesmtime_daily(thzm_xr)
    thzm_xr = thzm_xr.where( thzm_xr.time.dt.hour == 12, drop=True)
    thzm_xr = thzm_xr.sel(time=slice("1979-01-01","2023-12-31"))
    print('after THzm')

    filedir=basepath+'VTHzm/'
    vthzm_xr = xr.open_mfdataset(filedir+"/*-"+imem+"*.VTHzm.*.nc").sel(time=slice("1978-01-01","2024-12-31"))
    vthzm_xr = read.fixcesmtime_daily(vthzm_xr)
    vthzm_xr = vthzm_xr.where( vthzm_xr.time.dt.hour == 12, drop=True)
    vthzm_xr = vthzm_xr.sel(time=slice("1979-01-01","2023-12-31"))
    print('after VTHzm')

    filedir=basepath+'Vzm/'
    vzm_xr = xr.open_mfdataset(filedir+"/*-"+imem+"*.Vzm.*.nc").sel(time=slice("1978-01-01","2024-12-31"))
    vzm_xr = read.fixcesmtime_daily(vzm_xr)
    vzm_xr = vzm_xr.where( vzm_xr.time.dt.hour == 12, drop=True)
    vzm_xr = vzm_xr.sel(time=slice("1979-01-01","2023-12-31"))
    print('after Vzm')

    filedir=basepath+'UVzm/'
    uvzm_xr = xr.open_mfdataset(filedir+"/*-"+imem+"*.UVzm.*.nc").sel(time=slice("1978-01-01","2024-12-31"))
    uvzm_xr = read.fixcesmtime_daily(uvzm_xr)
    uvzm_xr = uvzm_xr.where( uvzm_xr.time.dt.hour == 12, drop=True)
    uvzm_xr = uvzm_xr.sel(time=slice("1979-01-01","2023-12-31"))
    print('after UVzm')

    filedir=basepath+'UWzm/'
    uwzm_xr = xr.open_mfdataset(filedir+"/*-"+imem+"*.UWzm.*.nc").sel(time=slice("1978-01-01","2024-12-31"))
    uwzm_xr = read.fixcesmtime_daily(uwzm_xr)
    uwzm_xr = uwzm_xr.where( uwzm_xr.time.dt.hour == 12, drop=True)
    uwzm_xr = uwzm_xr.sel(time=slice("1979-01-01","2023-12-31"))
    print('after UWzm')

    filedir=basepath+'Wzm/'
    wzm_xr = xr.open_mfdataset(filedir+"/*-"+imem+"*.Wzm.*.nc").sel(time=slice("1978-01-01","2024-12-31"))
    wzm_xr = read.fixcesmtime_daily(wzm_xr)
    wzm_xr = wzm_xr.where( wzm_xr.time.dt.hour == 12, drop=True)
    wzm_xr = wzm_xr.sel(time=slice("1979-01-01","2023-12-31"))
    print('after Wzm')
    ...
    intv = integrate.cumulative_trapezoid(vzmwithzero, prewithzero*100., axis=1)
