...
for imem in mems: 
    filedir=basepath+imem+"/atm/day_1/"
    uzm_xr = xr.open_mfdataset(filedir+"/*.Uzm.*.nc")
    uzm_xr = read.fixcesmtime_daily(uzm_xr)
    uzm_xr = uzm_xr.sel(time=slice("1979-01-01","2014-12-31"))
    uzm_xr = uzm_xr.chunk({'time':uzm_xr.time.size, 'ilev':uzm_xr.ilev.size, 'lat':uzm_xr.lat.size})
    thzm_xr = xr.open_mfdataset(filedir+"/*.THzm.*.nc").sel(time=slice("1978-01-01","2015-12-31"))
    thzm_xr = read.fixcesmtime_daily(thzm_xr)
    thzm_xr = thzm_xr.sel(time=slice("1979-01-01","2014-12-31"))
    vthzm_xr = xr.open_mfdataset(filedir+"/*.VTHzm.*.nc").sel(time=slice("1978-01-01","2015-12-31"))
    vthzm_xr = read.fixcesmtime_daily(vthzm_xr)
    vthzm_xr = vthzm_xr.sel(time=slice("1979-01-01","2014-12-31"))
    vzm_xr = xr.open_mfdataset(filedir+"/*.Vzm.*.nc").sel(time=slice("1978-01-01","2015-12-31"))
    vzm_xr = read.fixcesmtime_daily(vzm_xr)
    vzm_xr = vzm_xr.sel(time=slice("1979-01-01","2014-12-31"))
    uvzm_xr = xr.open_mfdataset(filedir+"/*.UVzm.*.nc").sel(time=slice("1978-01-01","2015-12-31"))
    uvzm_xr = read.fixcesmtime_daily(uvzm_xr)
    uvzm_xr = uvzm_xr.sel(time=slice("1979-01-01","2014-12-31"))
    uwzm_xr = xr.open_mfdataset(filedir+"/*.UWzm.*.nc").sel(time=slice("1978-01-01","2015-12-31"))
    uwzm_xr = read.fixcesmtime_daily(uwzm_xr)
    uwzm_xr = uwzm_xr.sel(time=slice("1979-01-01","2014-12-31"))
    wzm_xr = xr.open_mfdataset(filedir+"/*.Wzm.*.nc").sel(time=slice("1978-01-01","2015-12-31"))
    wzm_xr = read.fixcesmtime_daily(wzm_xr)
    wzm_xr = wzm_xr.sel(time=slice("1979-01-01","2014-12-31"))
    ...
    uzm.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc")
    epfy.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    epfz.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    vtem.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    wtem.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    psitem.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    utendepfd.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    utendvtem.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    utendwtem.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
    dudt.to_netcdf(outdir+"FWHIST_"+str(imem).zfill(3)+".nc", mode="a")
