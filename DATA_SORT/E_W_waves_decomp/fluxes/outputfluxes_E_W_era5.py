...
basepath="/project/cas/islas/python_savs/CAM7_vertres_paper/RAW_DATA/ERA5/"
pathout="/project/cas/islas/python_savs/CAM7_vertres_paper/DATA_SORT/E_W_waves_decomp/fluxes/"

u = xr.open_mfdataset(basepath+'U_dayfrom6h/U_*.nc').U
u = u.sel(lat=slice(-30,30))
print('read in U')
v = xr.open_mfdataset(basepath+'V_dayfrom6h/V_*.nc').V
v = v.sel(lat=slice(-30,30))
print('read in V')
w = xr.open_mfdataset(basepath+'W_dayfrom6h/W_*.nc').W
w = w.sel(lat=slice(-30,30))
print('read in W')
t = xr.open_mfdataset(basepath+'T_dayfrom6h/T_*.nc').T
t = t.sel(lat=slice(-30,30))
print('read in T')

u = u.sel(time=slice("1979-01-01","2023-12-31"))
v = v.sel(time=slice("1979-01-01","2023-12-31"))
w = w.sel(time=slice("1979-01-01","2023-12-31"))
t = t.sel(time=slice("1979-01-01","2023-12-31"))
...
ds.load().to_netcdf(pathout+'fluxes_E_W_stat_era5.nc')
