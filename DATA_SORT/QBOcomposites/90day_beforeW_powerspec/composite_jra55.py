...
for ip in pre:
    for ilatbnd in latbnd:
        dat = xr.open_mfdataset(basepath+"*.nc").sel(pre=ip, lat=slice(-1.*ilatbnd-2, ilatbnd+2))
        u = dat.u.load()
        w = dat.w.load()
        ...
        dat.to_netcdf(pathout+'JRA55_'+str(ip)+'hpa_'+str(ilatbnd)+'S'+str(ilatbnd)+'N_90days.nc')
