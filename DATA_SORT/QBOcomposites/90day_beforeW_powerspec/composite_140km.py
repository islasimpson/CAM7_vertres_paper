...
pathout="/project/cas/islas/python_savs/CAM7_vertres_paper/DATA_SORT/QBOcomposites/60day_beforeW_powerspec/"
...
for iexp in exp:
    ...
    for ip in pre:
        for ilatbnd in latbnd:
            ...
            dat_zmfluxes = xr.open_dataset("/project/cas/islas/python_savs/CAM7_vertres_paper/DATA_SORT/"+
               "E_W_waves_decomp/fluxes/fluxes_E_W_stat_"+iexp+"_140km.nc")
            dat_zmfluxes = dat_zmfluxes.sel(pre=ip, method='nearest')

            u = xr.open_mfdataset("/project/cas/islas/python_savs/L83_paper/CAM7_vertres_paper/140km/"+
               "day/plev/day_from_6h/"+iexp+"/U_*.nc")\
               .sel(pre=ip, lat=slice(-1.*ilatbnd-2, ilatbnd+2)).load()

            w = xr.open_mfdataset("/project/cas/islas/python_savs/L83_paper/CAM7_vertres_paper/140km/"+
...
#                dat_tem = dat_tem.sel(time=slice(str(ystart)+'-01-01',str(yend)+'-12-31'))
            ...
            dat.to_netcdf(pathout+iexp+'_'+str(ip)+'hpa_'+str(ilatbnd)+'S'+str(ilatbnd)+'N.nc')
